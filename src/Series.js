...
const Series = () => {
    ...
    const getSeries = async() => {
        try {
            ...
            const url = getUrl('series',12,parseInt(pageNum) * 12);
    ...
    return (
        <ListLayout> 
          <Stack direction='horizontal' className='mb-2 top-btn-list' >
                ...
                <SearchInput placeholder='series' handleSearch={value => {
                ...
                {(searchParams.get('query') === "" ||!searchParams.get('query')) && list  && <Stack className='ms-auto list-btns' gap={2} direction='horizontal'>
                    {pageNum >= 1 && <Button disabled={list.length === 0} onClick={() => navigate(`/series/page/${parseInt(pageNum) - 1}`, {replace:true})}>Prev</Button>}
                    {hasMore && <Button disabled={list.length === 0} onClick={() => navigate(`/series/page/${parseInt(pageNum) + 1}`, {replace:true})}>Next</Button>}
                </Stack>}
            </Stack>
            <div className='list'>
                {list && list.length > 0 ? list.map((el) => 
                    <ListItemCard key={el.id} image={`${el.thumbnail.path}.${el.thumbnail.extension}`} name={el.title} desc={el.description} navigate={() => navigate(`/series/${el.id}`, {replace:true})}/>
                    )
                    :
                    [1,2,3,4].map(i => 
